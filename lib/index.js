...
// return Promise
Crowdin.prototype.fileUpload = function(uri, path, formData) {
  var filename = path.substring(path.lastIndexOf('/') + 1);
  if (!formData) {
      formData = {};
  }
  formData.key=  this.config.apiKey
  formData['files[' + filename + ']'] = fs.createReadStream(path);
...
// return Promise
Crowdin.prototype.addFile = function(path, options) {
    return this.fileUpload('add-file', path, options);
};

// return Promise
Crowdin.prototype.updateFile = function(path, options) {
    return this.fileUpload('update-file', path, options);
};
